...
import { Search, Trash2, Loader2, Sparkles, MessageSquare, Star, Filter } from "lucide-react";
...
export default function ManageReviewsPage() {
  ...
  const [deletingReview, setDeletingReview] = useState<AdminReviewDTO | null>(null);
  ...
  const filtered = reviews.filter(r => {
    // Filter by type
    if (filter === "event" && r.targetType.toLowerCase() !== "event") return false;
    if (filter === "place" && r.targetType.toLowerCase() !== "place") return false;
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setReviews(prev => prev.map(r => r.id === deletingReview.id ? { ...r, isDeleted: true } : r));
    ...
    <div className="space-y-10 max-w-[1400px] mx-auto pb-12 relative z-0">
      ...
      >
        <div className="relative">
          <div className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-amber-50 border border-amber-100 mb-4">
            ...
            <span className="text-amber-700 text-sm font-['Inter:SemiBold',sans-serif]">Review Moderation</span>
          </div>
          ...
          <p className="text-slate-500 font-['Inter:Medium',sans-serif] mt-3 text-[16px] max-w-xl">
            Monitor community feedback across all events and venues. Identify and remove abusive or inappropriate reviews.
          </p>
        </div>
        ...
        <div className="flex flex-col items-end gap-4 w-full lg:w-auto">
          ...
          <div className="relative flex bg-slate-100/80 p-1.5 rounded-xl backdrop-blur-md border border-slate-200">
            ...
            <button
              ...
              className={`relative z-10 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-['Inter:SemiBold',sans-serif] w-28 transition-colors ${
                filter === "event" ? "text-slate-900" : "text-slate-500 hover:text-slate-700"
              }`}
            >
              Events
            </button>
            <button
              ...
              className={`relative z-10 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-['Inter:SemiBold',sans-serif] w-28 transition-colors ${
                filter === "place" ? "text-slate-900" : "text-slate-500 hover:text-slate-700"
              }`}
            >
              Venues
            </button>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32 bg-white/50 backdrop-blur-xl rounded-[2rem] border border-white shadow-sm">
          ...
          <p className="text-slate-600 font-['Inter:Bold',sans-serif] mt-4 tracking-wide text-sm">FETCHING REVIEWS</p>
        </div>
      ) : filtered.length === 0 ? (
        <motion.div 
        ...
        >
          ...
          <h3 className="text-slate-900 font-['Inter:Bold',sans-serif] text-[20px] mb-2 tracking-tight">No Reviews Found</h3>
            ...
            {filtered.map((review, i) => (
              <motion.div
                ...
                className={`group relative bg-white rounded-[2rem] p-6 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.08)] hover:shadow-[0_20px_40px_-12px_rgba(0,0,0,0.12)] hover:-translate-y-1 border transition-all duration-300 flex flex-col ${
                  review.isDeleted ? 'border-red-200 bg-red-50/30' : 'border-slate-100'
                }`}
              >
                {/* Status Badges */}
                <div className="absolute top-6 right-6 flex flex-col gap-2 items-end">
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[12px] font-['Inter:Bold',sans-serif] shadow-sm ${
                ...
                <div className="flex items-start gap-4 mb-4 pr-20">
                  ...
                  <div className="flex-1">
                    <h3 className="text-[16px] leading-tight font-['Inter:Bold',sans-serif] text-slate-900 mb-0.5 truncate">{review.reviewerName}</h3>
                ...
                <div className="bg-slate-50/70 rounded-2xl p-4 border border-slate-100/80 mb-5">
                  <p className="text-[11px] font-['Inter:SemiBold',sans-serif] text-slate-400 uppercase tracking-wider mb-1">Target</p>
                  <p className="text-[14px] font-['Inter:Bold',sans-serif] text-slate-800 truncate">{review.targetName}</p>
                </div>

                <div className="flex-1 mb-6">
                  <p className="text-slate-600 font-['Inter:Regular',sans-serif] text-[14px] leading-relaxed line-clamp-4 relative">
                    <span className="text-3xl font-serif text-slate-200 absolute -top-3 -left-2">"</span>
        ...
        {deletingReview && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              ...
              <h2 className="text-2xl font-['Inter:Bold',sans-serif] text-slate-900 mb-3">Delete Review</h2>
              <p className="text-slate-500 font-['Inter:Medium',sans-serif] text-[15px] mb-8 leading-relaxed">
                Are you sure you want to delete this review by <strong>{deletingReview.reviewerName}</strong>? This action cannot be undone.
              </p>
              ...
              <div className="flex flex-col gap-3">
                ...
                >
                  {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : "Yes, delete review"}
                </button>
                <button 
                  disabled={submitting}
