...
export default function OwnerProfilePage() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        ...
        const profile = await ownerApi.getProfile(ownerId!);
        if (!isActive) return;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedProfile = await ownerApi.updateProfile(ownerId!, updateDto);
      ...
      if (pendingRemove) {
        await ownerApi.deleteProfilePicture(ownerId!);
        setProfilePictureUrl(null);
        localStorage.removeItem("forsa_profile_picture");
        window.dispatchEvent(new Event("profilePictureUpdated"));
      } else if (pendingImageFile) {
        const url = await ownerApi.uploadProfilePicture(ownerId!, pendingImageFile);
              ...
              <div>
                ...
                <div className="relative group">
                  <input
                    ...
                    className="w-full pl-4 pr-12 py-3 rounded-xl border border-slate-200 text-sm focus:border-[#1E3D61] focus:ring-2 focus:ring-[#1E3D61]/10 outline-none transition-all text-slate-800 text-left"
