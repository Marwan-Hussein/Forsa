...
import { Search, Calendar, MapPin, CheckCircle, XCircle, Clock, Check, X, CalendarCheck, Loader2, ClipboardList } from "lucide-react";
          ...
          {tabs.map(tab => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${
                activeTab === tab.key ? tab.activeClass : `${tab.color} hover:bg-white/60`
              }`}
            >
              {tab.label}
              <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded-md ${
                activeTab === tab.key ? "bg-white/80 text-slate-700 shadow-sm" : "bg-slate-200/80 text-slate-500"
              }`}>
                {counts[tab.key]}
              </span>
            </button>
                      ...
                      <div className="sm:hidden space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2.5">
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center border text-sm font-bold flex-shrink-0 ${
                              sk === "pending" ? "bg-amber-50 border-amber-200 text-amber-600" :
                      ...
                      <div className="hidden sm:grid grid-cols-[1.2fr_1.4fr_100px_110px_170px] gap-4 items-center">
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-sm font-bold flex-shrink-0 border ${
                            sk === "pending" ? "bg-amber-50 border-amber-200 text-amber-700" :
