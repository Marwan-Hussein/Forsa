...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [statsData, reqsData, reviewsData] = await Promise.all([
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      >
        <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-gradient-to-br from-blue-50/30 to-transparent rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none" />

        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 relative z-10">
          <div>
            <h2 className="text-xl font-bold text-slate-800 tracking-tight">Organizer Feedback & Reviews</h2>
            <p className="text-slate-500 text-sm mt-1 font-medium">Ratings left by organizers for your venues</p>
          </div>
          <Link to="/owner/reviews" className="inline-flex items-center gap-1.5 text-sm font-bold text-blue-600 hover:text-blue-700 transition-colors bg-blue-50 px-4 py-2 rounded-xl">
            View All Reviews <ArrowRight className="w-4 h-4" />
          </Link>
        </div>

        {reviews.length === 0 ? (
          <div className="text-center py-10 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200 relative z-10">
            <MessageSquare className="w-12 h-12 text-slate-300 mx-auto mb-2" />
            <p className="text-sm text-slate-500 font-medium">No reviews received yet.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 relative z-10">
            {/* Left: Star Distribution Summary */}
            <div className="lg:col-span-2 space-y-6 bg-slate-50/50 p-6 rounded-3xl border border-slate-100 flex flex-col justify-center">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-450 uppercase">{reviews.length} total reviews</p>
                </div>
              </div>
                ...
                {[5, 4, 3, 2, 1].map((stars) => {
                  ...
                  return (
                    <div key={stars} className="flex items-center gap-2 text-xs font-semibold text-slate-500">
                      ...
                      <span className="w-12 text-right text-[10px] text-slate-400">{count} ({percentage.toFixed(0)}%)</span>
                    </div>
                  );
                })}
              </div>
            ...
            {/* Right: Latest 3 Reviews */}
            <div className="lg:col-span-3 space-y-4">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-2">Latest Feedback</h3>
              <div className="space-y-3">
                {reviews.slice(0, 3).map((review) => (
                  <div key={review.id} className="p-4 rounded-2xl bg-white border border-slate-100 shadow-sm flex gap-4 items-start">
                    <div className="w-9 h-9 rounded-full bg-blue-50 border border-blue-100 flex items-center justify-center shrink-0 text-blue-600 font-bold text-xs">
                      {review.organizerName.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <span className="text-sm font-bold text-slate-800 truncate">{review.organizerName}</span>
                      ...
                      <div className="flex items-center gap-1.5 mb-1.5">
                        ...
                        <span className="text-[10px] text-slate-400 truncate max-w-[150px]">• {review.placeName}</span>
                      </div>
