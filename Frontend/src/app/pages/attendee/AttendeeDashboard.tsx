...
function EventCardVertical({ event, onToggleWishlist, isInWishlist }: { event: Event; onToggleWishlist: (id: string) => void; isInWishlist: boolean }) {
  ...
  return (
    <Link to={`/events/${event.id}`} className="group block cursor-pointer">
      <div className="bg-white rounded-2xl border border-[#526d82]/10 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-200">
        <div className="relative h-40">
          ...
          >
            <Heart className={`w-4 h-4 ${isInWishlist ? 'fill-[#ec4899] text-[#ec4899]' : 'text-[#526d82]'}`} />
          </button>
        ...
        <div className="p-4">
          <h3 className="font-semibold text-[#27374d] line-clamp-1">{event.title}</h3>
          <p className="text-xs text-[#526d82] line-clamp-2 mt-1 mb-2">{event.description}</p>
          <div className="flex items-center justify-between text-xs text-[#526d82]">
          ...
          <div className="flex items-center justify-between mt-3">
            <span className="font-bold text-[#27374d]">${event.price}</span>
            <span className="text-xs text-[#526d82]">{event.attendees} going</span>
          </div>
        </div>
      </div>
    </Link>
...
function EventCardHorizontal({ event, onToggleWishlist, isInWishlist }: { event: Event; onToggleWishlist: (id: string) => void; isInWishlist: boolean }) {
  ...
  return (
    <Link to={`/events/${event.id}`} className="group block cursor-pointer">
      <div className="bg-white rounded-xl border border-[#526d82]/10 p-3 flex gap-3 hover:shadow-md transition-all">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between">
            <div>
              <h3 className="font-semibold text-[#27374d] text-sm line-clamp-1">{event.title}</h3>
              <p className="text-xs text-[#526d82] line-clamp-1 mt-0.5">{event.description}</p>
            </div>
          ...
          <div className="flex items-center gap-3 mt-2 text-xs text-[#526d82]">
            <div className="flex items-center gap-1">
              <ClockIcon className="w-3 h-3" />
              <span>{formattedDate}</span>
            </div>
          ...
          <div className="flex items-center justify-between mt-1">
            <span className="font-semibold text-[#27374d] text-sm">${event.price}</span>
            <span className="text-xs text-[#526d82]">{event.attendees} going</span>
          </div>
        </div>
      </div>
...
export default function AttendeeDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#eff6ff] via-white to-[#eff6ff]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {/* Header with avatar and search */}
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-[#27374d] rounded-2xl flex items-center justify-center text-white font-bold text-xl">
              {userName[0]}
            </div>
            <div>
              <h1 className="text-2xl font-bold text-[#27374d]">Welcome back, {userName}!</h1>
          ...
          <div className="flex items-center gap-3">
            <button className="p-2 cursor-pointer text-[#526d82] hover:text-[#27374d] hover:bg-white rounded-xl transition-all">
              <Bell className="w-5 h-5" />
            </button>
            <div className="relative hidden sm:block">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#526d82]" />
              <input
                type="text"
                placeholder="Search events..."
                className="pl-9 pr-4 py-2 bg-white border border-[#526d82]/20 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-[#155dfc]/20 focus:border-[#155dfc] transition-all w-64"
              />
            </div>
          </div>
        </div>
        ...
        {featuredEvent && (
          ...
          >
            <Link to={`/events/${featuredEvent.id}`} className="group block cursor-pointer">
              <div className="relative h-64 overflow-hidden rounded-3xl shadow-[0_20px_50px_-20px_rgba(39,55,77,0.35)] ring-1 ring-white/10 transition-[transform,box-shadow] duration-300 ease-in-out group-hover:-translate-y-1 group-hover:shadow-[0_28px_60px_-18px_rgba(39,55,77,0.45)] md:h-80">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#27374d]/80 via-transparent to-transparent" />
                <div className="absolute bottom-0 left-0 p-6 text-white">
                  <span className="bg-[#EC9B3B] text-white text-xs font-semibold px-3 py-1 rounded-full mb-3 inline-block">
                    Featured
                  </span>
        ...
        <div className="mb-10 flex flex-wrap gap-3">
          {quickStats.map((stat, index) => (
            ...
            >
              <Link
                to={stat.link}
                className="flex items-center gap-2 rounded-full border border-[#526d82]/10 bg-white/95 px-4 py-2 shadow-sm backdrop-blur-sm transition-shadow duration-300 ease-in-out hover:border-[#526d82]/20 hover:shadow-md cursor-pointer"
              >
                <stat.icon className="h-4 w-4" style={{ color: stat.color }} />
                <span className="text-sm font-medium text-[#27374d]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-[#27374d]">Your upcoming</h2>
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-[#EC9B3B]" />
                <h2 className="text-lg font-semibold text-[#27374d]">Recommended for you</h2>
              </div>
        ...
        <div className="mt-12">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-[#27374d]">Trending this week</h2>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-12">
          <Link
            to="/organizations"
            className="bg-white rounded-xl border border-[#526d82]/10 p-5 hover:shadow-md transition-all flex items-center gap-4 cursor-pointer"
          >
            ...
            <div className="flex-1">
              <p className="font-semibold text-[#27374d]">Organizations</p>
              <p className="text-xs text-[#526d82]">Follow your favorites</p>
            </div>
          ...
          >
            ...
            <div className="flex-1">
              <p className="font-semibold text-[#27374d]">Wishlist</p>
              <p className="text-xs text-[#526d82]">{wishlist.length} saved events</p>
            </div>
            <ChevronRight className="w-4 h-4 text-[#526d82]" />
          </Link>
          ...
          >
            ...
            <div className="flex-1">
              <p className="font-semibold text-[#27374d]">My events</p>
              <p className="text-xs text-[#526d82]">View your bookings</p>
            </div>
            <ChevronRight className="w-4 h-4 text-[#526d82]" />
          </Link>
        </div>
